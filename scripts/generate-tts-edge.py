...
VOICES using a worker pool. Writes:
  ...
  lib/tts/audio-manifest.ts   (preserves non-Edge voice entries such as Gemini)
...
EDGE_IDS = set(VOICES.keys())


def parse_non_edge_voices(manifest_path: Path) -> list[dict]:
    """Return voice entries from audio-manifest.ts that are NOT Edge voices.

    Called before regenerating the manifest so that non-Edge voices (e.g. the
    Gemini voice added by generate-tts-gemini.py) are preserved verbatim.
    ...
    try:
        ...
        existing: list[dict] = json.loads(array_text)
        return [v for v in existing if v["id"] not in EDGE_IDS]
...
async def main() -> None:
    ...
    # Preserve any non-Edge voices (e.g. Gemini) already in the manifest.
    non_edge_voices = parse_non_edge_voices(MANIFEST)
    ...
    # Re-append non-Edge voices after Edge voices.
    voices_manifest.extend(non_edge_voices)

    manifest_ts = (
        "/** @generated by scripts/generate-tts-edge.py"
        + (" and scripts/generate-tts-gemini.py" if non_edge_voices else "")
        + " \u2014 do not edit. */\n"
