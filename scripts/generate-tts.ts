...
const OUT_DIR = join(ROOT, "public", "tts", "Gemini");
const HASHES_PATH = join(ROOT, "scripts", "gemini-hashes.json");
...
/**
 * The Gemini API returns raw 16-bit little-endian PCM (audio/l16).
 * Wrap it in a WAV container so browsers can play it natively.
 */
...
async function main() {
  ...
  if (existsSync(HASHES_PATH)) {
    const previous = await readFile(HASHES_PATH, "utf8");
    const existing = JSON.parse(previous) as string[];
  ...
  await writeFile(HASHES_PATH, JSON.stringify(generated, null, 2) + "\n");

  console.log(
    `Done: ${done} generated, ${skipped} skipped, ${failed} failed. Total Gemini clips: ${generated.length}`,
  );
}
